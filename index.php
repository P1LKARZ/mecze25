  ...
  <div class="container text-center" id="tabelka">
    <table class="table table-hover">
      ...
      if (!empty($_POST['wybor_gospodarzy']) && !empty($_POST['wybor_gosci']) && !empty($_POST['wybor_poziom']) && !empty($_POST['wybor_sedzia']) && !empty($_POST['nr_meczu']) && !empty($_POST['data'])) {
          ...
          $wstawianie = "INSERT INTO mecz VALUES (NULL, '$gospodarz_', '$gosc_', '$poziom_', '$kwota', '$podatek', '$data_', '$sedzia_', $nr_meczu_, '$platnosc', '$zaplacone')";
      ...
      while ($r = mysqli_fetch_array($kw_wyswietl_tabela)) {
          ...
          if ($r[10] == "N") {
              echo "<td><form method='post' style='display:inline;'>
                      <input type='hidden' name='id_zaplac' value='{$r['id_mecz']}'>
                      <button type='submit' class='btn btn-success' name='zaplacone' >✔</button>
                    </form></td>";
                    if(isset($_POST['zaplacone'])) {
                      $id_zaplac = intval($_POST['id_zaplac']);
                      $zaplac_query = "UPDATE mecz SET zaplacone = 'T' WHERE id_mecz = $id_zaplac";
                      $zaplac_result = mysqli_query($conn, $zaplac_query);
                      if ($zaplac_result) {
                          echo "<script>
                                  alert('Zapłacono!');
                          ...
                          echo "<script>
                                  alert('Błąd podczas płacenia. Spróbuj ponownie.');
                                </script>";
                      }
                    }
          } else {
